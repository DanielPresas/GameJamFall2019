	...
	:Cappuccino::Scene(isActive), _in(true, std::nullopt), cursorBox(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(100.0f, 100.0f, 100.0f)), startBox(glm::vec3(0.0f, -75.0f, 0.0f), glm::vec3(200.0f, 100.0f, 200.0f))
{
	...
	ui._uiComponents.push_back(new Cappuccino::UIText("T O O  M A N Y  C H A R A C T E R S", glm::vec2(1600.0f, 1200.0f), *_uiShader, glm::vec2(-400.0f, 600.0f), glm::vec3(1.0f, 1.0f, 1.0f), 2.5f));
	menuShader->use();
...
void MenuScene::childUpdate(float dt)
{
	...
	dynamic_cast<Cappuccino::UIText*>(ui._uiComponents.back())->setTextColour(glm::vec3(meme, 0.0f, meme));
