...
GameScene::GameScene(bool firstScene) : Scene(firstScene)
{
	...
	bullet = new Projectile(&dirLight._dirLightShader, std::vector<Cappuccino::Texture*>{new Cappuccino::Texture("nut.png", Cappuccino::TextureType::DiffuseMap)}, std::vector<Cappuccino::Mesh*>{new Cappuccino::Mesh("projectile.obj")});
	...
	//));
	enemies.push_back(new Enemy(dirLight._dirLightShader, std::vector<Texture*>{ new Texture("Kratos.png", TextureType::DiffuseMap), new Texture("Sans.jpg", TextureType::SpecularMap) }, std::vector<Mesh*>{ new Mesh("humanoid2.obj") }, new Pistol(bullet)));
